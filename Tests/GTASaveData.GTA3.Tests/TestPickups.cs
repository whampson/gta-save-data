    ...
    public class TestPickups : Base<Pickups>
    {
        public override Pickups GenerateTestObject(SaveFileFormat format)
        {
            Faker<Pickups> model = new Faker<Pickups>()
                .RuleFor(x => x.PickupsArray, f => Generator.CreateArray(Pickups.Limits.NumberOfPickups, g => Generator.Generate<Pickup, TestPickup>()))
                .RuleFor(x => x.LastCollectedIndex, f => f.Random.Short())
                .RuleFor(x => x.PickupsCollected, f => Generator.CreateArray(Pickups.Limits.NumberOfCollectedPickups, g => f.Random.Int()));
        ...
        public void Serialization()
        {
            ...
            Assert.Equal(x0.PickupsArray, x1.PickupsArray);
