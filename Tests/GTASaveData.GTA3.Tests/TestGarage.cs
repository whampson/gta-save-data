    ...
    public class TestGarage : Base<Garage>
    {
        public override Garage GenerateTestObject(DataFormat format)
        {
            Faker<Garage> model = new Faker<Garage>()
                ...
                .RuleFor(x => x.PositionMin, f => Generator.Generate<Vector, TestVector>())
                .RuleFor(x => x.PositionMax, f => Generator.Generate<Vector, TestVector>())
                .RuleFor(x => x.DoorOpenOffset, f => f.Random.Float())
                .RuleFor(x => x.DoorOpenMax, f => f.Random.Float())
                .RuleFor(x => x.Door1Position, f => Generator.Generate<Vector, TestVector>())
                .RuleFor(x => x.Door2Position, f => Generator.Generate<Vector, TestVector>())
